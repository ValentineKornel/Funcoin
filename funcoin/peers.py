...
class P2PProtocol:
    ...
    async def handle_ping(self, message, writer):
        ...
        if block_height < self.blockchain.last_block["height"]:
            #Send them each block in succession, form their height
    ...
    async def hadle_transaction(self, message, writer):
        """
        ...
        if validate_transaction(tx) is True:
            #Add the tx to our pool, and propagate it to out peers
    ...
    async def handle_peers(self, message, writer):
        ...
        #Craft a ping message for us to sed to each peer
        ...
        for peer in peers:
            ...
            #We are only interested int eh "writer"
